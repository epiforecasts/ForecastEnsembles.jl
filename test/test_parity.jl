@testitem "Parity — hubEnsembles::simple_ensemble" begin
    ...
    # mean
    out_df = select(DataFrame(combine(ft, QuantileEnsemble(:mean))), on_cols..., :value)
    ref = select(
    ...
    # median
    out_med = select(DataFrame(combine(ft, QuantileEnsemble(:median))), on_cols..., :value)
    ref_med = select(
...
@testitem "Parity — hubEnsembles::linear_pool (quantile)" begin
    ...
    ft = ForecastTable(in_df; task_id_cols = [:location, :horizon])
    out_df = select(
        DataFrame(combine(ft, LinearPool())),
        :location,
        :horizon,
        :output_type_id,
        :value
    ...
    # are R's Monte Carlo noise (n_output_samples = 10,000 in the fixture;
    # quantile SE ≲ 0.01 on this ~N(·, 1.3) scale) and the spline-vs-PCHIP
    # interior. Observed max diff ≈ 0.02; 0.05 leaves a 2.5× margin.
