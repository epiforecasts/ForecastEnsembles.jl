@testitem "QuantileEnsemble" begin
    ...
    # unweighted mean
    out = combine(ft, QuantileEnsemble(:mean))
    d = DataFrame(out)
    ...
    # unweighted median
    out_med = combine(ft, QuantileEnsemble(:median))
    ...
    out_wm = combine(ft, QuantileEnsemble(:median; weights = w))
    ...
    @test_throws ArgumentError combine(ft, QuantileEnsemble(:mean; weights = w_bad))

    # invalid agg
    @test_throws ArgumentError QuantileEnsemble(:max)
end
